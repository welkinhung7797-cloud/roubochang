...
function updateProjectiles(dt) {
  ...
  for (const b of G.projectiles) {
    ...
    if (b.cut) {
      // ★ 被刀斬開的半片：改打敵人，不再傷害玩家（動畫漫畫的套路）
      for (const e of G.enemies) {
        if (e.dead || e === b.owner) continue;
        const r2 = e.r + b.r;
        if (dist2(e.x, e.y, b.x, b.y) < r2 * r2) {
          hurtEnemy(e, techDmg(b.dmg * 1.6), { crit: chance(0.35), fromAngle: Math.atan2(b.vy, b.vx) });
          burstParticles(b.x, b.y, { n: 5, spd: 130, life: 0.3, size: 2, col: '#8fd4e0' });
          b.dead = true; break;
        }
      }
      if (b.dead) continue;
      burstParticles(b.x, b.y, { n: 1, spd: 12, life: 0.16, size: 1.4, col: '#8fd4e0', grav: 0 });
      continue;
    }
    const rr = p.r + b.r;
